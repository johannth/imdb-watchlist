...
export const checkIfMovieIsOnLocalNetflix = (imdbId, title, locale) => {
	...
	const startRequest = () =>
		fetch(
			`http://${
				locales[locale]
			}.flixlist.co/autocomplete/titles?q=${encodeURIComponent(title)}`
		)
			.then(handleErrors)
...
const doubleCheckIfMovieIsOnLocaleNetflix = (imdbId, netflixUrl, locale) => {
	...
	const netflixUrlInLocaleAlternative = netflixUrl.replace(
		"/title/",
		`/${locale}-en/title/`
	);

	console.log(netflixUrl, netflixUrlInLocale, netflixUrlInLocaleAlternative);
	...
	return new Promise((resolve, reject) => {
		request(
			...
			(error, response, body) => {
				...
				console.log(
					`/api/netflix ${imdbId}: Netflix returned ${
						response.statusCode
					} on ${netflixUrl} with location ${locationHeader}`
				);
