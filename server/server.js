...
const fetchMovieDetails = movie =>
  Promise.all([
    ...
    checkIfMovieIsOnLocalNetflixWithCache(movie.id, movie.title, 'is').catch((error) => {
    ...
    .then((bechdelRating_justWatch_localNetflixUrl) => {
      ...
      const { viewingOptions, ratings } = bechdelRating_justWatch_localNetflixUrl[1];
      ...
      return movieData({
        ...
        showtime: viewingOptions && viewingOptions.showtime,
      });
    })
    .catch((error) => {
      console.error(error);
...
const fetchJustWatchDataWithCache = (imdbId, title, type, releaseDateTimestamp) => {
